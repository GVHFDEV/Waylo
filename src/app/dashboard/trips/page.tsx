...
import { MapPin, CalendarDays, Users, Wallet, Plus, ArrowRight } from 'lucide-react'
    ...
    <div className="space-y-8">
      ...
      {trips && trips.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          ...
          <Link href="/dashboard/wizard" className="group block">
            <article className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-transparent min-h-[200px] transition-all duration-300 hover:border-primary hover:bg-primary/5">
              <Plus className="h-8 w-8 text-muted-foreground group-hover:text-primary transition-colors mb-2" />
              <span className="text-sm font-bold text-muted-foreground group-hover:text-primary transition-colors font-heading">
                {lang === 'pt' ? 'Criar Nova Viagem' : lang === 'es' ? 'Crear Nuevo Viaje' : lang === 'fr' ? 'Créer un Voyage' : lang === 'de' ? 'Neue Reise erstellen' : lang === 'it' ? 'Crea Nuovo Viaggio' : lang === 'ja' ? '新しい旅行を作成' : 'Create New Trip'}
              </span>
            </article>
          </Link>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 md:py-24 text-center space-y-6">
          ...
          <Link href="/dashboard/wizard">
            <Button className="bg-primary hover:bg-primary-hover text-white font-bold h-12 px-8 rounded-xl shadow-lg shadow-primary/20">
